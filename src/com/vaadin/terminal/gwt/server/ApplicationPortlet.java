...
/**
 * Portlet main class for Portlet 1.0 (JSR-168) portlets which consist of a portlet and a servlet. For Portlet 2.0 (JSR-286, no servlet required), use {@link ApplicationPortlet2} instead.
 */
...
public class ApplicationPortlet implements Portlet, Serializable {
	...
	protected void writeAjaxWindow(RenderRequest request, RenderResponse response) throws IOException {
		...
		if (app != null) {
			...
			try {
				...
				request.setAttribute(AbstractApplicationServlet.REQUEST_FRAGMENT, "true");

				// fixed base theme to use - all portal pages with Vaadin
				// applications will load this exactly once
				String portalTheme = getPortalProperty(Constants.PORTAL_PARAMETER_VAADIN_THEME, portalCtx);

				String portalWidgetset = getPortalProperty(Constants.PORTAL_PARAMETER_VAADIN_WIDGETSET, portalCtx);

				// location of the widgetset(s) and default theme (to which
				// /VAADIN/widgetsets/...
				// is appended)
				String portalResourcePath = getPortalProperty(Constants.PORTAL_PARAMETER_VAADIN_RESOURCE_PATH, portalCtx);
				...
				if (portalResourcePath != null) {
					...
					// servlet
					request.setAttribute(AbstractApplicationServlet.REQUEST_VAADIN_STATIC_FILE_PATH, portalResourcePath);
				}
				...
				if (portletWidgetset != null) {
					request.setAttribute(AbstractApplicationServlet.REQUEST_WIDGETSET, portletWidgetset);
				}
				if (portalWidgetset != null) {
					request.setAttribute(AbstractApplicationServlet.REQUEST_SHARED_WIDGETSET, portalWidgetset);
				}

				if (style != null) {
					request.setAttribute(AbstractApplicationServlet.REQUEST_APPSTYLE, style);
				}

				// portalTheme is only used if the shared portal resource
				// directory is defined
				if (portalTheme != null && portalResourcePath != null) {
					request.setAttribute(AbstractApplicationServlet.REQUEST_DEFAULT_THEME, portalTheme);

					String defaultThemeUri = null;
					defaultThemeUri = portalResourcePath + "/" + Constants.THEME_DIRECTORY_PATH + portalTheme;
					/*
					 * Make sure portal default Vaadin theme is included in DOM. Vaadin portlet themes do not "inherit" base theme, so we need to force loading of the common base theme.
					 */
				...
				if (isLifeRay) {
					/*
					 * Temporary support to heartbeat Liferay session when using Vaadin based portlet. We hit an extra xhr to liferay servlet to extend the session lifetime after each Vaadin request. This hack can be removed when supporting portlet 2.0 and resourceRequests.
					 * 
					 * TODO make this configurable, this is not necessary with some custom session configurations.
					 */
					OutputStream out = response.getPortletOutputStream();

					String lifeRaySessionHearbeatHack = "<script type=\"text/javascript\">" + "if(!vaadin.postRequestHooks) {" + "    vaadin.postRequestHooks = {};" + "}" + "vaadin.postRequestHooks.liferaySessionHeartBeat = function() {" + "    if (Liferay && Liferay.Session && Liferay.Session.setCookie) {" + "        Liferay.Session.setCookie();" + "    }" + "};" + "</script>";
	...
	public void init(PortletConfig config) throws PortletException {
		...
		if (app == null) {
			throw new PortletException("No porlet application url defined in portlet.xml. Define the '" + PORTLET_PARAMETER_APPLICATION + "' init parameter to be the servlet deployment path.");
		}
