 ...
 * 
 */
public class VRichTextArea extends Composite implements Paintable, Field, ChangeHandler, BlurHandler, KeyPressHandler, KeyDownHandler, BeforeShortcutActionListener, Focusable {
	...
	private void selectAll() {
		...
		/*
		 * There is a timing issue if trying to select all immediately on first render. Simple deferred command is not enough. Using Timer with moderated timeout. If this appears to fail on many (most likely slow) environments, consider increasing the timeout.
		 * 
		 * FF seems to require the most time to stabilize its RTA. On Vaadin tiergarden test machines, 200ms was not enough always (about 50% success rate) - 300 ms was 100% successful. This however was not enough on a sluggish old non-virtualized XP test machine. A bullet proof solution would be nice, GWT 2.1 might however solve these. At least setFocus has a workaround for this kind of issue.
		 */
		new Timer() {
	...
	public void onKeyDown(KeyDownEvent event) {
		...
		if (shortcutHandler != null) {
			shortcutHandler.handleKeyboardEvent(com.google.gwt.user.client.Event.as(event.getNativeEvent()), this);
		}
	}
	...
	public void setFocus(boolean focused) {
		...
		/*
		 * Similar issue as with selectAll. Focusing must happen before possible selectall, so keep the timeout here lower.
		 */
		new Timer() {
	...
	public void setHeight(String height) {
		...
		if (height == null || height.equals("")) {
			...
			/*
			 * The formatter height will be initially calculated wrong so we delay the height setting so the DOM has had time to stabilize.
			 */
			Scheduler.get().scheduleDeferred(new Command() {
	...
	public void setWidth(String width) {
		...
		if (width.endsWith("px")) {
			...
			/*
			 * IE cannot calculate the width of the 100% iframe correctly if there is no width specified for the parent. In this case we would use the toolbar but IE cannot calculate the width of that one correctly either in all cases. So we end up using a default width for a RichTextArea with no width definition in all browsers (for compatibility).
			 */
