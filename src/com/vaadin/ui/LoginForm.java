...
public class LoginForm extends CustomComponent {
	...
	private ApplicationResource loginPage = new ApplicationResource() {
		...
		public DownloadStream getStream() {
			...
			return new DownloadStream(new ByteArrayInputStream(getLoginHTML()), getMIMEType(), getFilename());
		}
	};
	...
	private URIHandler uriHandler = new URIHandler() {
		...
		public DownloadStream handleURI(URL context, String relativeUri) {
			...
			if (relativeUri != null && relativeUri.contains("loginHandler")) {
				...
				DownloadStream downloadStream = new DownloadStream(new ByteArrayInputStream(response.getBytes()), "text/html", "loginSuccesfull");
	...
	static {
		try {
			ON_LOGIN_METHOD = LoginListener.class.getDeclaredMethod("onLogin", new Class[] {
				LoginEvent.class
			});
	...
	public void setHeight(float height, int unit) {
		...
		if (iframe != null) {
			if (height < 0) {
				...
				iframe.setHeight(T4JConstants.FULL_WIDTH_PERCENTAGE);
			}
		}
	}
	...
	public void setWidth(float width, int unit) {
		...
		if (iframe != null) {
			if (width < 0) {
				...
				iframe.setWidth(T4JConstants.FULL_WIDTH_PERCENTAGE);
			}
		}
	}
