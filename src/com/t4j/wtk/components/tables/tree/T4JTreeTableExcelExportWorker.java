...
import com.t4j.wtk.components.tables.plain.T4JTableColumnGenerator;
...
public class T4JTreeTableExcelExportWorker implements Runnable, Serializable {

	private static final long serialVersionUID = 1L;

	private static final Log logger = T4JLogUtils.getLogger(T4JTreeTableExcelExportWorker.class);
	...
	protected T4JTreeTableComponent tableComponent;
	...
	public T4JTreeTableExcelExportWorker() {

		this(null, null, null);
	}

	public T4JTreeTableExcelExportWorker(T4JTreeTableComponent tableComponent) {

		this(tableComponent, null, null);
	}

	public T4JTreeTableExcelExportWorker(T4JTreeTableComponent tableComponent, Integer fromPage, Integer toPage) {
	...
	protected void addHeaderRow() {
		...
		currentSheet = workbook.createSheet(((T4JWebApp) tableComponent.getApplication()).getI18nString("T4JTableExcelExportWorker.sheet", "Hoja") + T4JConstants.WHITESPACE_STRING + sheetIndex);
	...
	protected T4JTreeTableComponent getTableComponent() {
				...
				try {
					...
					if (null == columnGenerator) {
						...
						if (columnGenerator instanceof T4JTableColumnGenerator) {
							...
							((T4JTableColumnGenerator) columnGenerator).generateExcelCell(currentRow, cellIndex, tableComponent.getTreeTable(), itemId, columnDescriptor.getPropertyId());
							isExcelCellGeneratedValue = true;
						} else {

							currentCell = currentRow.createCell(cellIndex);
							propertyValue = columnGenerator.generateCell(tableComponent.getTreeTable(), itemId, columnDescriptor.getPropertyId());
				...
				if (false == isExcelCellGeneratedValue) {
					...
					if (null == propertyValue) {
						...
						if (propertyValue instanceof Label) {
							...
							currentCell.setCellValue(caption);
						}
					} else {
						currentCell.setCellValue(T4JConstants.EMPTY_STRING);
					}
		...
		synchronized (tableComponent.getApplication()) {
			...
			if (null == threadException) {
				...
				if (Boolean.TRUE.equals(isInterrupted)) {
					tableComponent.showExcelExportInterruptedNotification(destinationFile);
				} else {
					tableComponent.showExcelExportFileDownloadDialog(destinationFile);
				}
			} else {
				...
				if (Boolean.TRUE.equals(isInterrupted)) {
					...
					tableComponent.showExcelExportErrorNotification(destinationFile);
				}
			}
		}
	}
	...
	protected void updateProgressIndicator() {
		...
		synchronized (tableComponent.getApplication()) {
			...
			tableComponent.updateExcelExportProgressIndicatorValue(Float.valueOf(1.0f * processedItemsCount / totalItemsCount));
		}
	}
	...
	public void setTableComponent(T4JTreeTableComponent tableComponent) {
